...
async fn main() {
    ...
    // Parse the URL into a Tokio Postgres configuration.
    let client_config: PgClientConfig = database_url.parse().expect("Invalid DB URL");

    // Create a deadpool Postgres config and manually set fields.
    ...
    // (Optionally set other fields such as application_name, options, etc.)

    let pool: Pool = pg_cfg
        .create_pool(Some(Runtime::Tokio1), NoTls)
        .expect("Error creating pool");

    if let Err(e) = initialise_experiment(&pool).await {
        eprintln!("Error: {}", e);
    }
}
