...
pub const EFFECT_CODON_SIZE: usize = 8; // Size of effect codon in bits
...
pub const LOW_PASS_CODON: [u8; EFFECT_CODON_SIZE] = [1, 0, 1, 0, 1, 0, 1, 1];
pub const HIGH_PASS_CODON: [u8; EFFECT_CODON_SIZE] = [0, 1, 0, 1, 0, 1, 0, 0];
pub const REVERB_CODON: [u8; EFFECT_CODON_SIZE] = [1, 1, 1, 0, 1, 1, 1, 1];
pub const ECHO_CODON: [u8; EFFECT_CODON_SIZE] = [0, 0, 0, 1, 0, 0, 0, 0];
...
impl Genome {
    ...
    pub fn initialise_random_genome() -> Self {
        ...
        let left_chromosome = (0..LEFT_CHROMOSOME_SIZE).map(|_| rng.gen_range(0..=1)).collect();
        let right_chromosome = (0..RIGHT_CHROMOSOME_SIZE).map(|_| rng.gen_range(0..=1)).collect();
        Genome {
            left_chromosome,
            right_chromosome,
        }
